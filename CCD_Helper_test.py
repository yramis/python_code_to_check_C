...
class CCSD_Helper(object):
    ...
    def calculate_charge(self, F):        
        ...
        #HF_p = np.dot(C[:,o],C[:,o].T)
        HF_p = contract('ik,jk->ij', C[:, o], np.conj(C[:, o]))
        #HF_p = self.pholowdinbasis(HF_p)
        S = self.S
        nmo = self.nmo
        ...
        D_dot_s = contract('sr,rp->sp', HF_p, S)
        qO = 0
        print(HF_p.shape)
        for q in range(8):
            print( D_dot_s[q,q])
            qO += D_dot_s[q,q]
        print( "The charge on oxygen is: ", qO)
        #qO = 16-2*qO
        qO = (8-qO)/2.0
        print( "The charge on oxygen is: ", qO)

    def localize_two_orbitals(self):
        nmo = self.nmo
        H1 = slice(0, 1)
        H2 = slice(1, 2)
        Oxy = slice(3, 5)
        ...
        def Cal_s_Pa_t(H1, S):
            CH1 = C[H1, All]
            SH1 = S[All, H1]
            s_Pa_t_H1  = 0.5*contract('ps,pu,ut->st', C, SH1, CH1)
            SH1 = S[H1, All]
            s_Pa_t_H1 += 0.5*contract('us,up,pt->st', CH1, SH1, C)
            return s_Pa_t_H1
        
        def Cal_t_Pa_t(H1, S):
            CH1 = C[H1, All]
            SH1 = S[All, H1]
            t_Pa_t_H1  = 0.5*contract('pt,pu,ut->t', C, SH1, CH1)
            SH1 = S[H1, All]
            t_Pa_t_H1 += 0.5*contract('ut,up,pt->t', CH1, SH1, C)
            return t_Pa_t_H1
        
        def Cal_s_Pa_t_H2O(S):
            SH1 = S[All, Oxy]
            SH2 = S[All, H2]
            SH1 = S[All, H1]
            s_Pa_t  = Cal_s_Pa_t(H1, S)
            s_Pa_t += Cal_s_Pa_t(H2, S)
            s_Pa_t += Cal_s_Pa_t(Oxy, S)
            return s_Pa_t

        def Cal_t_Pa_t_H2O(S):
            SH1 = S[All, Oxy]
            SH2 = S[All, H2]
            SH1 = S[All, H1]
            t_Pa_t  = Cal_t_Pa_t(H1, S)
            t_Pa_t += Cal_t_Pa_t(H2, S)
            t_Pa_t += Cal_t_Pa_t(Oxy, S)
            return t_Pa_t

        def cal_Ast(s,t, s_Pa_t, t_Pa_t):
            return s_Pa_t[s][t] -(1/4)*(t_Pa_t[s] - t_Pa_t[t]  )**2.0

        def cal_Bst(s,t, s_Pa_t, t_Pa_t):
            return s_Pa_t[s][t]*(t_Pa_t[s] - t_Pa_t[t])

        def sin_cos(s,t):
            Ast = cal_Ast(s,t, s_Pa_t, t_Pa_t)
            Bst = cal_Bst(s,t, s_Pa_t, t_Pa_t)
            cos4alpha = -Ast/np.sqrt(Ast**2 + Bst**2)
            x1 = 1/2*(1 + np.sqrt(1-1/2*(1-cos4alpha)))
            y1 = np.sqrt(1-x1)**2
            test1 = 4*x1*y1*(x1**2 -y1**2) 
            x2 = 1/2*(1 - np.sqrt(1-1/2*(1-cos4alpha)))
            y2 = np.sqrt(1-x2)**2
            test2 = 4*x2*y2*(x2**2 -y2**2) 
            sin4alpha = Bst/np.sqrt(Ast**2 + Bst**2)
            if test1 == sin4alpha:
                print( "test1 matches")
                return x1, y1
            elif test2 == sin4alpha:
                print( "test2 matches")
                return x1, y1
            else:
                print( "no match")
        ...
        s_Pa_t = Cal_s_Pa_t_H2O(self.S)
        t_Pa_t = Cal_t_Pa_t_H2O(self.S)
        s = 2
        t = 3
        for x in range(10):
            x, y = sin_cos(s,t)
